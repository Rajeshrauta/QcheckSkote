...
import { Table, SearchResult } from '../utilities/customer-transaction.model';
import { tableData } from '../utilities/data';
import { SortDirection } from '../utilities/customer-sortable.directive';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.type.toLowerCase().includes(term.toLowerCase())
        || tables.createddate.toLowerCase().includes(term)
        || tables.id.toLowerCase().includes(term)
        || tables.transactiondate.toLowerCase().includes(term)
        || pipe.transform(tables.balance).includes(term)
        || tables.Comments.toLowerCase().includes(term);
}
