...
import { Table, SearchResult } from '../utilities/products.model';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term.toLowerCase())
        || tables.code.toLowerCase().includes(term)
        || tables.sku.toLowerCase().includes(term)
        || tables.desc.toLowerCase().includes(term)
        || tables.price.toLowerCase().includes(term)
        || pipe.transform(tables.qtyinhand).includes(term)
        || tables.sku.toLowerCase().includes(term);
}
